...
const DATA_COLOR = '#E06A1C';
const AVG_COLOR  = '#E6C36C';
  ...
  const perDay = useMemo(() => {
    ...
    return days.map((d) => {
      ...
      for (const log of logs) {
        // 🔧 Нормализуем createdAt к полуночи, чтобы сегодняшние логи не отбрасывались
        const created = safeDate(log?.createdAt, new Date(0));
        created.setHours(0,0,0,0);
       

        // 🔧 Если дни не выбраны — считаем, что тренировка относится к дню создания
